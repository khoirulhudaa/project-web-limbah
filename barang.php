            ...
            <div class="card-body">
              ...
              <hr>
                    <!-- Button trigger modal -->
                      ...
                      <a href="cetak_data_barang.php" target="_blank" class="btn btn-outline-info mb-3">
                        Print
                      </a>
              ...
              <table class="table table-bordered">
                ...
                <?php
                  ...
                  if (isset($_GET['search'])) {
                    ...
                    $sql = "SELECT * FROM tbl_barang WHERE jns_plastik LIKE '%$search_keyword%'";
                  ...
                  <tr>
                  <td><?php echo"$no"; ?></td>
                    <td><?php echo"$rs[kd_plastik]"; ?></td>
                    <td><?php echo"$rs[jns_plastik]"; ?></td>
                    <td><?php echo"$rs[stok]"; ?></td>
                    <td><?php echo"$rs[satuan]"; ?></td>
                    <td class="d-flex justify-content-center">
                      <!-- Button trigger modal -->
                    <button type="button" class="btn btn-outline-success mb-2" data-bs-toggle="modal" data-bs-target="#ModalEdit<?php echo $rs['id_barang']; ?>"><i class="bi bi-pencil"></i></button>
                     <!-- Modal Edit -->
                       <form method="POST" action="./barang.php">
                          <div class="modal fade" id="ModalEdit<?php echo $rs['id_barang']; ?>" tabindex="-1" aria-labelledby="ModalEdit" aria-hidden="true">
                              <div class="modal-dialog">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title" id="ModalEdit">Form Edit Data Barang</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                  </div>
                                <div class="modal-body">
                                    <div class="mb-3">
                                      <label for="kd_plastik" class="form-label">Kode Plastik</label>
                                      <input type="text" class="form-control" name="kd_plastik" value="<?php echo"$rs[kd_plastik]"?>">
                                    </div>
                                    <div class="mb-3">
                                        <label>Jenis plastik</label>
                                        <select name="jns_plastik" class="custom-select form-control">
                                        <option selected>Pilih jenis</option>
                                        <option value="ABS" <?php echo ($rs['jns_plastik'] === 'ABS') ? 'selected' : ''; ?>>ABS</option>
                                        <option value="HIPS" <?php echo ($rs['jns_plastik'] === 'HIPS') ? 'selected' : ''; ?>>HIPS</option>
                                        <option value="ADROLIK" <?php echo ($rs['jns_plastik'] === 'ADROLIK') ? 'selected' : ''; ?>>ADROLIK</option>
                                        <option value="POM" <?php echo ($rs['jns_plastik'] === 'POM') ? 'selected' : ''; ?>>POM</option>
                                        <option value="NILONIPA" <?php echo ($rs['jns_plastik'] === 'NILONIPA') ? 'selected' : ''; ?>>NILONIPA</option>
                                        <option value="PVC" <?php echo ($rs['jns_plastik'] === 'PVC') ? 'selected' : ''; ?>>PVC</option>
                                        <option value="PC" <?php echo ($rs['jns_plastik'] === 'PC') ? 'selected' : ''; ?>>PC</option>
                                        <option value="PET" <?php echo ($rs['jns_plastik'] === 'PET') ? 'selected' : ''; ?>>PET</option>
                                        <option value="PS" <?php echo ($rs['jns_plastik'] === 'PS') ? 'selected' : ''; ?>>PS</option>
                                        <option value="AS" <?php echo ($rs['jns_plastik'] === 'AS') ? 'selected' : ''; ?>>AS</option>
                                        <option value="HD" <?php echo ($rs['jns_plastik'] === 'HD') ? 'selected' : ''; ?>>HD</option>
                                        <option value="LD" <?php echo ($rs['jns_plastik'] === 'LD') ? 'selected' : ''; ?>>LD</option>
                                        <option value="HD BLOW" <?php echo ($rs['jns_plastik'] === 'HD BLOW') ? 'selected' : ''; ?>>HD BLOW</option>
                                        <option value="DAILON" <?php echo ($rs['jns_plastik'] === 'DAILON') ? 'selected' : ''; ?>>DAILON</option>
                                        <option value="ELA" <?php echo ($rs['jns_plastik'] === 'ELA') ? 'selected' : ''; ?>>ELA</option>
                                        <option value="KARESIN" <?php echo ($rs['jns_plastik'] === 'KARESIN') ? 'selected' : ''; ?>>KARESIN</option>
                                        <option value="ASTROLOID" <?php echo ($rs['jns_plastik'] === 'ASTROLOID') ? 'selected' : ''; ?>>ASTROLOID</option>
                                        <option value="HIP" <?php echo ($rs['jns_plastik'] === 'HIP') ? 'selected' : ''; ?>>HIP</option>
                                        <option value="TPU" <?php echo ($rs['jns_plastik'] === 'TPU') ? 'selected' : ''; ?>>TPU</option>
                                        <option value="TPR" <?php echo ($rs['jns_plastik'] === 'TPR') ? 'selected' : ''; ?>>TPR</option>
                                        <option value="Dan lainnya" <?php echo ($rs['jns_plastik'] === 'DAN LAINNYA') ? 'selected' : ''; ?>>DAN LAINNYA</option>
                                        </select>
                                    </div>
                                    <div class="mb-3">
                                      <label for="stok" class="form-label">Stok</label>
                                      <input type="text" class="form-control" name="stok" value="<?php echo"$rs[stok]"?>" disabled>
                                    </div>
                                    <div class="mb-3">
                                        <label>Satuan</label>
                                        <select name="satuan" class="custom-select form-control">
                                        <option selected>Pilih satuan</option>
                                        <option value="Kg" <?php echo ($rs['satuan'] === 'Kg') ? 'selected' : ''; ?>>Kg</option>
                                        <option value="Ton" <?php echo ($rs['satuan'] === 'Ton') ? 'selected' : ''; ?>>Ton</option>
                                        <option value="Kwintal" <?php echo ($rs['satuan'] === 'Kwintal') ? 'selected' : ''; ?>>Kwintal</option>
                                        </select>
                                      </div>
                                </div>
                                <div class="modal-footer">
                                  <input type="hidden" name="id_barang" class="form-control" value="<?php echo"$rs[id_barang]" ?>">
                                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        ...
                        if (isset($_POST['update'])) {
                          ...
                          $sqls = " UPDATE tbl_barang SET kd_plastik='$kd_plastik', jns_plastik='$jns_plastik', satuan='$satuan' WHERE id_barang='$id_barang'";
                        ...
                        ?>
                    <!-- End Modal Edit -->
                  <a href ="?page=barang&hapus=<?php echo $rs['id_barang']; ?>" class="btn btn-outline-danger" style="height: 38px;margin: 0 10px;" name="hapus"><i class="bi bi-trash"></i></a>
                  <?php
                   if (isset($_GET['hapus'])) {
                    $id_barang = $_GET['hapus'];
                    if (!empty($id_barang)) {
                      $sqlhapus = "DELETE FROM tbl_barang WHERE id_barang='$id_barang'";
