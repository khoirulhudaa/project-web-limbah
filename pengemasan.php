        ...
        if($_POST['kd_plastik'] == "") {
            ...
            if($sql2) {
              echo '<script>setTimeout(function(){ location.reload(); }, 100);</script>';  echo '<script>
                      setTimeout(function() {
                          window.location.href = "?page=pengemasan";
                      }, 100);
                    </script>';
              $conn->close();
              exit;
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Kelola Pengemasan</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
            ...
            <div class="card-body">
              ...
              <hr>
                    <!-- Button trigger modal -->
                      ...
                      <a href="cetak_data_pengemasan.php" target="_blank" class="btn btn-outline-info mb-3">
                        Print</a>
                ...
                <?php
                  $no=1;
                      if (isset($_GET['search'])) {
                        ...
                        $sql = "SELECT * FROM tbl_pengemasan WHERE nama_brg LIKE '%$search_keyword%'";
                        $sqls = mysqli_query($conn, $sql);
                      } else {
                        $sql = "SELECT * FROM tbl_pengemasan";
                        $sqls = mysqli_query($conn, $sql);
                    }
                  ...
                  <tr>
                  <td><?php echo"$no"; ?></td>
                    <td><?php echo"$rs[id_pengemasan]"; ?></td>
                    ...
                    <td class="d-flex justify-content-center">
                      <!-- Button trigger modal -->
                    <button type="button" class="btn btn-outline-success mb-2" data-bs-toggle="modal" data-bs-target="#ModalEdit<?php echo $rs['id_pengemasan']; ?>"><i class="bi bi-arrow-right"></i></button>
                     <!-- Modal Edit -->
                       <form method="POST" action="" enctype="multipart/form-data">
                          <div class="modal fade" id="ModalEdit<?php echo $rs['id_pengemasan']; ?>" tabindex="-1" aria-labelledby="ModalEdit" aria-hidden="true">
                              <div class="modal-dialog">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title" id="ModalEdit">Form produksi (pengemasan)</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                  </div>
                                <div class="modal-body">
                                    <div class="mb-3">
                                      <label for="kd_plastik" class="form-label">Id pengemasan</label>
                                      <input type="text" class="form-control" name="id_pengemasan" value="<?php echo"$rs[id_pengemasan]"?>">
                                    </div>
                                    <div class="mb-3">
                                      <label for="kd_plastik" class="form-label">Kode plastik</label>
                                    ...
                                    <div class="mb-3" style="transform: scale(0.1);opacity: 0;position: absolute;z-index; -1;">
                                    ...
                                    <div class="mb-3">
                                      <label for="stok" class="form-label">Nama pengirim</label>
                                      <input type="text" class="form-control" name="nama_pengirim2">
                                    </div>
                                    <div class="mb-3">
                                      <label for="stok" class="form-label">alamat pengirim</label>
                                      <input type="text" class="form-control" name="alamat_pengirim2">
                                    </div>
                                    <div class="mb-3">
                                        <label for="stok" class="form-label">Tujuan pengiriman</label>
                                        <input type="text" class="form-control" name="tujuan2">
                                    </div>
                                    <div class="mb-3">
                                      <label for="stok" class="form-label">Foto barang</label>
                                      <br />
                                      <input type="file" name="foto_brg">
                                    </div>
                                </div>
                                <div class="modal-footer">
                                  <input type="hidden" name="id_pengemasan" class="form-control" value="<?php echo"$rs[id_pengemasan]" ?>">
                                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    ...
                    if (isset($_POST['produksi'])) {
                      $id_pengemasan = $_POST['id_pengemasan'];
                      $kd_plastik = $_POST['kode_brg2'];
                      $nama_brg = $_POST['nama_brg2'];
                      $nama_pengirim = $_POST['nama_pengirim2'];
                      $alamat_pengirim = $_POST['alamat_pengirim2'];
                      $jum_sortir = $_POST['jum_sortir2'];
                      $tujuan = $_POST['tujuan2'];
                      $status = "Progress";
                      $foto_brg = $_FILES['foto_brg']['name'];
                      $tmp_foto = $_FILES['foto_brg']['tmp_name'];
                      $uniqueFileName = uniqid() . '.' . pathinfo($_FILES['foto_brg']['name'], PATHINFO_EXTENSION);
                      $foto_path = "./uploads/" . $uniqueFileName;
                      ...
                      $id_pengiriman = generateRandomString(5);
                      ...
                      if ($_POST['kode_brg2'] == "") {
                          $error = "Kode plastik tidak boleh kosong!";
                            ...
                            $allowedTypes = array('image/jpeg', 'image/png', 'image/jpeg', 'image/svg');
                            if (in_array($_FILES['foto_brg']['type'], $allowedTypes)) {
                                $sql = "INSERT INTO tbl_pengiriman (id_pengiriman, kode_brg, nama_pengirim, alamat_pengirim, jum_kirim, nama_brg, tujuan, foto_brg, status) VALUES ('$id_pengiriman', '$kd_plastik', '$nama_pengirim', '$alamat_pengirim', '$jum_sortir', '$nama_brg', '$tujuan', '$uniqueFileName', '$status')";
                                $sql2 = mysqli_query($conn, $sql);
                                move_uploaded_file($tmp_foto, $foto_path);
                            if ($sql2) {
                                $sqldelete = "DELETE FROM tbl_pengemasan WHERE id_pengemasan = '$id_pengemasan'";
                    ...
                    ?>
                    <!-- End Modal Edit -->
                  <a href ="?page=barang&hapus=<?php echo $rs['id_pengemasan']; ?>" class="btn btn-outline-danger" style="height: 38px;margin: 0 10px;" name="hapus"><i class="bi bi-trash"></i></a>
                  <?php
                   if (isset($_GET['hapus'])) {
                       $id_pengemasan = $_GET['hapus'];
                       if (!empty($id_pengemasan)) {
                      $sqlhapus = "DELETE FROM tbl_pengemasan WHERE id_pengemasan='$id_pengemasan'";
